...
(Cocode NUMBER Constraint fk_fiscal_year References GL_Company (Cocode), Coyear number(4), Comonthid number(2), Comonthname varchar2(9), Pfrom date, Pto date, Initial_Year number(1), Year_Closed number(1), Month_Closed number(1), TYE_Executed date, CONSTRAINT GL_FISCAL_YEAR_PK PRIMARY KEY (Cocode,Coyear,Comonthid) ENABLE);
...
(Cocode NUMBER Constraint fk_cost_center References GL_Company (Cocode), Cccode varchar2(5), Cctitle varchar2(25), Cclevel number(1), CONSTRAINT GL_COST_CENTER_PK PRIMARY KEY (Cocode,Cccode) ENABLE);

--S5-- CHART OF ACCOUNT TABLE
CREATE TABLE GL_COA
(Cocode NUMBER Constraint fk_coa References GL_Company (Cocode), COAcode varchar2(11), COAtitle varchar2(50), COAlevel number(1), COAnature varchar2(11), COAtype varchar2(11), Cccode varchar2(5), CONSTRAINT GL_COA_PK PRIMARY KEY (Cocode,COAcode) ENABLE);

--S6A-- TRANSACTION MASTER TABLE
create table GL_TRAN_MASTER
(Tran_No number, Cocode number Constraint fk_tran_master1 References GL_Company (Cocode) NOT NULL, Coyear number(4) NOT NULL, comonthid number(2) NOT NULL, vchcode number Constraint fk_tran_master2 References GL_Voucher (vchcode) NOT NULL, vchno number(10) NOT NULL, vchdate date NOT NULL, vchdescription varchar2(150) NOT NULL, createdby varchar2(10) NOT NULL, createdon date NOT NULL, vchverified varchar2(1) NOT NULL, vchposted varchar2(1) NOT NULL, closing number(1) NOT NULL, constraint pk_tran_master PRIMARY KEY (tran_no), constraint fk_tran_master3 Foreign Key (Cocode,Coyear,Comonthid) References GL_Fiscal_Year);

--S6B-- TRANSACTION DETAIL TABLE
create table GL_TRAN_DETAIL
(Line_No number, Tran_No number NOT NULL, Cocode number Constraint fk_tran_detail1 References GL_Company (Cocode) NOT NULL, coacode varchar2(11) NOT NULL, cccode varchar2(5), vchdescription varchar2(150) NOT NULL, vchdr number(15,2) NOT NULL, vchcr number(15,2) NOT NULL, vchreference varchar2(25), reconciled number(1) NOT NULL, constraint pk_tran_detail Primary Key (line_no), constraint fk_tran_detail3 Foreign Key (cocode,cccode) References GL_Cost_Center, constraint fk_tran_detail4 Foreign Key (cocode,coacode) References GL_COA);

--S6C-- ADD FOREIGN KEY
ALTER TABLE GL_TRAN_DETAIL ADD CONSTRAINT FK_TRAN_DETAIL2 FOREIGN KEY (TRAN_NO) REFERENCES GL_TRAN_MASTER(TRAN_NO) ON DELETE CASCADE ENABLE;
...
(sr_no number, Cocode number Constraint fk_banks_os1 References GL_Company (Cocode) NOT NULL, coacode varchar2(11) NOT NULL, remarks varchar2(50) NOT NULL, vchdr number(15,2) NOT NULL, vchcr number(15,2) NOT NULL, reconciled number(1) NOT NULL, constraint pk_banks_os Primary Key (sr_no), constraint fk_banks_os2 Foreign Key (cocode,coacode) References GL_COA);
...
create table GL_BUDGET (cocode number Constraint fk_budget1 References GL_Company (Cocode) NOT NULL, coyear number(4), coacode varchar2(11) NOT NULL, coanature varchar2(11) NOT NULL, cccode varchar2(5), budget_amount1 number(15,2), budget_amount2 number(15,2), budget_amount3 number(15,2), budget_amount4 number(15,2), budget_amount5 number(15,2), 
...
budget_amount12 number(15,2), criterion number(1), constraint fk_budget2 Foreign Key (cocode,coacode) References GL_COA);
...
(groupID NUMBER(4) Constraint fk_Group_Detail References GL_GROUPS_MASTER (groupID), segmentID NUMBER Constraint fk_user_groups References GL_SEGMENTS (segmentID), segmentParent number, segmentType varchar2(4), pageID number(4), itemRole varchar2(10), allow_access varchar2(1));

--S16-- APPLICATION USERS TABLE
create table GL_USERS
(userID varchar2(50), cocode number Constraint fk_users References GL_Company (Cocode), coyear number(4), comonthid number(2), groupID number(4) Constraint fk_users2 References GL_GROUPS_MASTER (groupID), password varchar2(4000), admin varchar2(1), CONSTRAINT GL_USERS_PK PRIMARY KEY (userID) ENABLE);
